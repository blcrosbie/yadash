...
Four complete dashboards ship in [`examples/`](examples/) — each one a
self-contained CSV/YAML pair, not a toy. Clone the repo, run
`npx @blcrosbie/yadash dev examples/<file>.yaml`, and try one of these prompts
on whichever agent has the skill installed:

| dashboard | data | try asking |
|---|---|---|
| [`sales.yaml`](examples/sales.yaml) | meetings booked/held, pipeline, close rate by rep | "add a filter for deals over $10k and show it in the KPI row" |
| [`support.yaml`](examples/support.yaml) | ticket volume, first response, CSAT by team | "break the CSAT KPI down by channel instead of team" |
| [`ecommerce.yaml`](examples/ecommerce.yaml) | orders, revenue, channel and category mix | "add a scatter of units vs revenue by product" |
| [`marketing.yaml`](examples/marketing.yaml) | ad spend, CTR/CVR, ROAS, CAC by campaign | "flag any campaign with CAC over $60 in the table" |

A few more prompts worth trying, since they exercise different parts of the
DSL than "add a filter":

- "turn the revenue trend into a stacked bar by category instead" — swaps a
  chart type without touching the data or metrics.
- "the AOV KPI should turn red under $50" — adds a `thresholds` block.
- "this is getting cluttered, split channel mix into its own dashboard" —
  tests the fleet-refactoring guidance in
  [`references/refactoring.md`](skills/yadash/references/refactoring.md).
- "point this at my own CSV instead" — hand it a real export; the metrics,
  thresholds and layout carry over, only `data.*.path` and field names change.

The synthetic data for `ecommerce.yaml` and `marketing.yaml` is generated by
[`scripts/generate-example-data.mjs`](scripts/generate-example-data.mjs) — a
seeded, zero-dependency generator (no `faker` package) you can copy as a
starting point for your own example datasets.
